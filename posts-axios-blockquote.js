...
const totalPages = 7; 
const filename = './wp-posts-blockquote.json';
const promises = [];
let posts = [];
let postsList = [];
...
const getPosts = async (page) => {    
  try {
    ...
    const result = response.data.map(post => {
      ...
      // Separate chorus from verses (if exists)
      if(body.search("<blockquote>") !== -1){     
        temp = content.split("<blockquote><p>");        
        content = temp[0].concat("[[CHORUS-1]]");
        // some hymns, like hymn 32 or 525 have 2 chorus
        // (https://harpacrista.org/hino/525-vencendo-vem-jesus/)      
        if(temp.length === 3) {                    
          let temp2 = temp[2];                  
          temp = temp[1].split("</p></blockquote>");
          chorus.push(temp[0]);
          //if(number === 32) console.log(temp[1]);
          content = content.concat(temp[1]).concat("[[CHORUS-2]]")          
          chorus.push(temp2.replace("</p></blockquote>",""));          
          num_chorus = 2;
        } else {
          temp = temp[1].split("</p></blockquote>");
          chorus.push(temp[0]);
          content = content.concat(temp[1]) 
          num_chorus = 1;
        }
      }      

      return { number, title, slug, author, full_content, content, chorus, num_chorus };
    })
